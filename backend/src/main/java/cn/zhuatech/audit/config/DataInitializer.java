...
package cn.zhuatech.audit.config;
import cn.zhuatech.audit.domain.DomainCatalog;
import cn.zhuatech.audit.model.*;
import cn.zhuatech.audit.repository.*;
...
@Configuration public class DataInitializer {
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "AUDIT-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="AUDIT-DEMO-"+String.format("%03d",sequence);
